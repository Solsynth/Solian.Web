...
{:else if !realm}
	<div class="w-full max-w-md rounded-3xl border border-base-300/70 bg-base-100/90 p-8 shadow-2xl backdrop-blur-xl">
		<div class="alert alert-error"><span>{error || 'Failed to load invite'}</span></div>
	</div>
{:else}
	<div class="w-full max-w-lg overflow-hidden rounded-3xl border border-base-300/70 bg-base-100/95 shadow-2xl backdrop-blur-xl">
		...
		<p class="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-base-content/60">You're invited</p>
		<div class="mb-6 flex items-center gap-3">
			{#if avatarUrl}
				<div class="avatar">
					<div class="h-14 w-14 rounded-2xl ring ring-base-300 ring-offset-2 ring-offset-base-100">
						<img src={avatarUrl} alt={realm.name} />
					</div>
				</div>
			{:else}
				<div class="avatar avatar-placeholder">
					<div class="h-14 w-14 rounded-2xl bg-primary text-primary-content ring ring-base-300 ring-offset-2 ring-offset-base-100">
		...
		{#if isMember}
			<button class="btn btn-success w-full" onclick={() => goto(`/realms/${realm.slug}`)}>
				<Check size={16} />
				Open Realm
			</button>
		{:else}
			<button class="btn btn-primary w-full" onclick={joinRealm} disabled={isJoining || isCheckingMembership}>
